...
export default function App() {
  ...
  return (
    <SafeAreaView style={ flex:1, padding:16 }>
      <ScrollView>
        <Text style={ fontSize:22, fontWeight:'600', marginBottom:12 }>Payments Lab (Adyen/Amazon Pay) – Mobile</Text>
        <Button title="Ping API" onPress={async()=>{
          try {
            const r = await fetch('http://10.0.2.2:4105/'); // Android emulator loopback
